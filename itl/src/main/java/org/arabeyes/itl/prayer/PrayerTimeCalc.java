...
public class PrayerTimeCalc {
    ...
    public static Dms getNorthQibla(Location loc) {
        ...
        double num, denom;
        num = Math.sin(Utils.degToRad(loc.getDegreeLong()) - Utils.degToRad(Utils.KAABA_LONG));
        denom = (Math.cos(Utils.degToRad(loc.getDegreeLat())) * Math.tan(Utils.degToRad(Utils.KAABA_LAT)))
                - (Math.sin(Utils.degToRad(loc.getDegreeLat())) *
                ((Math.cos((Utils.degToRad(loc.getDegreeLong()) - Utils.degToRad(Utils.KAABA_LONG))))));
